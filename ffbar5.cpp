/*
 ...
 This program randomly generates possible menus for a level 5 bar in Food Fantasy.
...
const int FORMULA_AMOUNT = 17;
const int HIDDEN_AMOUNT = 11;
...
private:
	...
	bool longislandicetea();
	bool cubalibre();
	bool smallboatinthemistno1();
	bool palmbeach();
	bool betweenthetwo();
	bool zombie();
	bool basilshreddedgin();
	bool cantaritos();
	bool thorns();
	bool fishbank();
	bool daijiri();
...
int main()
{
	...
	//end of program
	cout<<endl<<"The most profitable menu is menu "<<bestMenu<<", which generates "<<bestProfit<<" in profit."<<endl<<endl<<endl;
...
void drinks::displayIngredients()
{
	...
	cout<<endl<<endl<<"  coffee liqueur"<<endl;
	for(int i = 1; i <= FLAVORED_AMOUNT; i++)
		{
			cout<<setw(9)<<flavored[i];
		}
...
int drinks::selection(int amount, int& failures)
{
	...
	switch (drink)
	{
		...
		case 17:	makeable = vodkasour();
					break;
		case 18:	makeable = longislandicetea();
					break;
		...
		case 23:	makeable = zombie();
					break;
		case 24:	makeable = basilshreddedgin();
					break;
...
bool drinks::blueflame()
{
	if (base[2]+1 > BASE_CAP || other[8]+3 > OTHER_CAP)
	{
		//cout<<"Cannot make blue flame";
		return 0;
	}
	else
	{
...
bool drinks::longislandicetea()
{
	if (base[3]+1 > BASE_CAP || base[4]+1 > BASE_CAP || base[6]+1 > BASE_CAP || base[1]+2 > BASE_CAP)
	{
		//cout<<"Cannot make long Island Ice Tea";
		return 0;
	}
	else
	{
		cout<<"* long island ice tea";
		base[3] += 1;
		base[4] += 1;
		base[6] += 1;
		base[1] += 2;
